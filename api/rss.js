...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { workspace, workspace_id, collection, limit, tag } = req.query;
    ...
    const items = (data || []).map(function(a) {
      ...
      ${(a.tags || []).map(function(t) { return '<category>' + escapeXml(t) + '</category>'; }).join('\n      ')}
    </item>`;
    }).join('\n');

    var rssUrl = siteUrl + '/api/rss';
