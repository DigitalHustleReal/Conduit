...
export function Topbar() {
  ...
  return (
    <header className="h-13 bg-card/80 backdrop-blur-sm border-b border-border flex items-center px-5 gap-3 shrink-0">
      {/* Breadcrumb */}
      ...
      <div className="flex-1 max-w-md mx-auto">
        <div className="relative">
          ...
          <Input placeholder="Search or ask AI... (Ctrl+K)" className="h-8 text-xs bg-muted/50 pl-8 border-border/50 focus:border-violet-500/50" />
        </div>
      </div>

      {/* Credit Meter */}
      <Link href="/settings" className="flex items-center gap-2 px-3 py-1.5 bg-muted/50 rounded-lg border border-border/50 hover:border-violet-500/50 transition-colors cursor-pointer">
      ...
      {/* Agent Widget */}
      <Link href="/agents" className="flex items-center gap-1.5 px-2.5 py-1.5 bg-muted/50 rounded-lg border border-border/50 text-[11px] text-muted-foreground hover:border-violet-500/50 transition-colors">
        <span>{'\uD83E\uDD16'}</span>
      ...
      {/* User Avatar */}
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-cyan-400 flex items-center justify-center text-white text-xs font-bold cursor-pointer hover:shadow-lg hover:shadow-violet-500/20 transition-shadow">
        U
      </div>
    </header>
  );
