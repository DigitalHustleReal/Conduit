...
export function Topbar() {
  ...
  return (
    <header className="h-13 bg-transparent backdrop-blur-sm border-b border-slate-700/30 flex items-center px-5 gap-3 shrink-0">
      {/* Breadcrumb */}
      <div className="flex items-center gap-1.5 text-sm">
        <Link href="/dashboard" className="text-slate-500 hover:text-slate-200 transition-colors">
          Conduit
        </Link>
        {breadcrumbs.map((crumb, i) => (
          <span key={crumb.href} className="flex items-center gap-1.5">
            ...
            {i === breadcrumbs.length - 1 ? (
              <span className="font-medium capitalize text-slate-100">{crumb.label}</span>
            ) : (
              <Link href={crumb.href} className="text-slate-500 hover:text-slate-200 transition-colors capitalize">
                {crumb.label}
              </Link>
            )}
          </span>
      ...
      <div className="flex-1 max-w-md mx-auto">
        <div className="relative">
          ...
          <Input placeholder="Search or ask AI... (Ctrl+K)" className="h-8 text-xs bg-slate-800/40 pl-8 border-slate-700/30 focus:border-blue-500/50" />
        </div>
      </div>

      {/* Credit Meter */}
      <Link href="/settings" className="flex items-center gap-2 px-3 py-1.5 bg-slate-800/40 rounded-lg border border-slate-700/30 hover:border-blue-500/40 transition-colors cursor-pointer">
      ...
      {/* Agent Widget */}
      <Link href="/agents" className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800/40 rounded-lg border border-slate-700/30 text-[11px] text-slate-400 hover:border-blue-500/40 transition-colors">
        <span>{'\uD83E\uDD16'}</span>
        <span className={activeAgents > 0 ? 'text-emerald-400' : ''}>
          <span className={`inline-block w-1.5 h-1.5 rounded-full mr-1 ${activeAgents > 0 ? 'bg-emerald-400' : 'bg-slate-600'}`} />
          {activeAgents} active
        </span>
      </Link>
